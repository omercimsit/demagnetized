...
{
    // Procedural flood wave that chases the player through the corridor.
    // Gerstner waves + wall collision + VFX integration.
    ...
    public class FloodWaveSystem : MonoBehaviour
    {
        ...
        // TODO: _frontWaveWidth currently unused, should modulate vertex X offset at leading edge
        ...
        // per-vertex wall data, updated each frame
        private float[] _wallProximity;
        private Vector3[] _wallNormals;
        ...
        private void UpdateMovement()
        {
            ...
            // only Z matters for the corridor chase
        ...
        private void CheckWallDirection(int vertexIndex, Vector3 origin, Vector3 direction)
        {
            if (Physics.Raycast(origin, direction, out RaycastHit hit, _wallDetectionDistance, _wallLayers))
            {
                ...
                proximity = Mathf.Pow(proximity, 0.5f);
        ...
        {
            // fill in the rows we skipped with raycasts
            int sampleStep = Mathf.Max(1, _resolutionZ / _raycastsPerSide);

            for (int z = 0; z <= _resolutionZ; z++)
            {
                if (z % sampleStep == 0) continue;
        ...
        private void TriggerSplashes()
        {
            ...
            foreach (var pos in _splashPositions)
            {
                ...
                // prefer the controller, fall back to raw VFX
        ...
        private void UpdateAudio()
        {
            ...
            // slight pitch variation based on speed - sounds more alive
        ...
        // Temporary speed surge (used by scripted events)
        // TODO: expose a UnityEvent so designers can trigger this from Timeline
        ...
        // FIXME: teleporting mid-wave causes a one-frame mesh pop, need to reset vertices too
