...
{
    /// <summary>
    /// Clone System UI - Extracted from AAACloneSystem for modularity
    /// Handles all OnGUI rendering for the clone selection and status panels
    /// Optimized with cached styles to reduce GC allocation
    /// </summary>
    public class CloneSystemUI : MonoBehaviour
    {
        ...
        // Cached styles - created once, reused every frame
        ...
        // Clone type display data (cached reference)
        ...
        #region Style Initialization (Cached)
        ...
        private static void InitStyles()
        {
            ...
            // Card title style (for selection UI)
            ...
            // Icon style (for clone card icons)
        ...
        private void DrawCardContent(Rect cardRect, AAACloneSystem.CloneTypeData data, bool isSelected, bool isRecorded, int index, float alpha, Texture2D tex)
        {
            ...
            // Icon (cached style, no per-frame allocation)
        ...
        {
            // Cleanup static textures only if this is the last instance
            if (_bgTexture != null)
            {
                Destroy(_bgTexture);
                _bgTexture = null;
